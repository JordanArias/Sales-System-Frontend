...
export class CocinaComponent {
  ...
  handleKeyboardEvent2(event: KeyboardEvent) {
    console.log('Tecla presionada:', event.key); // ✅ Debería aparecer en la consola cada vez que presiones una tecla

    var total_pag = Math.ceil(this.lista_Ventas.length/6);
    console.log('total_pag :: ', total_pag);
    ...
    console.log('Página actual:', this.page_v); // ✅ Debería mostrar la página actual
  }
...
handleKeyboardEvent(event: KeyboardEvent) {
  switch (event.key) {
    ...
    case '3':
      ...
      break;
    // Agrega más atajos según necesites
  }
}
